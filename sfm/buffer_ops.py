...
# Setting whitepoint and blackpoint on buf
class GradeOp:

    def __init__(self, blackpoint, whitepoint, gamma):
    ...
    def grade(self, buf):
        ...
        if self.blackpoint != [0.0, 0.0, 0.0]:
            inv = lambda bp: 1.0/(1.0-bp) if bp != 0.0 else bp
            inv_blackpoint = [inv(bp) for bp in self.blackpoint]            
            oiio.ImageBufAlgo.sub(dst, dst, tuple(self.blackpoint))
            oiio.ImageBufAlgo.mul(dst, dst, tuple(inv_blackpoint))
            oiio.ImageBufAlgo.clamp(dst, dst, 0.0, float('inf'))
            
        if self.whitepoint != [1.0, 1.0, 1.0]:
            oiio.ImageBufAlgo.div(dst, dst, tuple(self.whitepoint))
            
        if self.gamma != [1.0, 1.0, 1.0]:
            oiio.ImageBufAlgo.pow(dst, dst, tuple([1/g for g in self.gamma]))            

        return dst
        
    def op(self, buf_list):
        return [self.grade(buf) for buf in buf_list]
